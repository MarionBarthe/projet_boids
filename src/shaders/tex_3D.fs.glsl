...
uniform vec3 u_light_pos_vs;        // Light position in view space
...
vec3 blinn_phong_lighting(vec3 normal, vec3 frag_pos) {
    vec3 light_dir = normalize(u_light_pos_vs - frag_pos); // Direction from fragment to light
    ...
    float distance = length(u_light_pos_vs - frag_pos);
    ...
    float linear = 0.0001;   // Very low linear coefficient
    float quadratic = 0.00001; // Very low quadratic coefficient

    float attenuation = 1.0 / (constant + linear * distance + quadratic * distance * distance);

    vec3 light_intensity = u_light_intensity * attenuation;
...
void main() {
    ...
    vec3 lighting = blinn_phong_lighting(normal, v_position_vs); // Calculate lighting
